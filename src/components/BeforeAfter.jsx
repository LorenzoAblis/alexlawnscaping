...
const BeforeAfter = () => {
  return (
    ...
    >
      ...
      <div className="mt-5 flex flex-row w-6/7 overflow-x-scroll snap-x snap-mandatory gap-5 text-center h-80 pr-5">
        {collages.map((collage, index) => (
          <img
            src={`/beforeAfter/${collage}.webp`}
            alt={collage}
            className="object-cover w-6/7 rounded-4xl m-auto shadow-[0.5rem_0.5rem_0rem_var(--color-secondary)] snap-center"
          />
        ))}
      </div>
      <div className="mt-10">
        ...
        {photoPairs.map((pair, index) => (
          <div className="mt-5 flex flex-row w-6/7 overflow-x-scroll snap-x snap-mandatory gap-5 text-center h-80 pr-5">
            <img
              ...
              alt={pair.before}
              className="object-cover w-6/7 rounded-4xl m-auto shadow-[0.5rem_0.5rem_0rem_var(--color-secondary)] snap-center h-7/8"
            />
            <img
